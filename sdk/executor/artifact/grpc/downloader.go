...
func (d downloader) DownloadArtifact(ctx context.Context, ref artifact.Ref, target io.Writer) error {
	if d.client == nil {
...
func (d downloader) DownloadSource(ctx context.Context, ref artifact.SourceRef, target io.Writer) error {
	if d.client == nil {
		return fmt.Errorf("制品下载客户端尚未初始化")
	}
	stream, err := d.client.DownloadProjectSource(ctx, &artifactv1.DownloadProjectSourceRequest{
		SourceId: ref.SourceID,
		Digest:   ref.Digest,
	})
	if err != nil {
		return fmt.Errorf("请求下载项目源码失败: %w", err)
	}
	for {
		chunk, receiveErr := stream.Recv()
		switch {
		...
		case errors.Is(receiveErr, io.EOF):
			return nil
		default:
			return fmt.Errorf("接收项目源码数据失败: %w", receiveErr)
		}
	}
}

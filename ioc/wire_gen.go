...
func InitSchedulerApplication(base *Base) *SchedulerApplication {
	...
	service := task.NewService(taskRepository, bindingService, runnerService)
	...
	hubs := sse.NewHubs()
	executionService := task.NewExecutionService(string2, taskExecutionRepository, service, logService, completeProducer, registry, bindingRegistry, artifactService, programService, runnerService, hubs)

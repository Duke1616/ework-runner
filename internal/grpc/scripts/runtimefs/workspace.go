...
func NewWorkspaceFactory(config WorkspaceConfig, access WorkspaceAccess) (*WorkspaceFactory, error) {
	if access == nil {
		return nil, fmt.Errorf("工作区访问策略不能为空")
	}
...
func (f *WorkspaceFactory) Create(options engine.WorkspaceOptions) (engine.Workspace, error) {
	...
	if err := f.access.PrepareRoot(f.config.Dir); err != nil {
	...
	ws := &workspace{root: root, access: f.access}
	if err = f.access.PrepareWorkspace(root); err != nil {
		_ = ws.Close()
		return nil, fmt.Errorf("准备任务工作区访问权限失败: %w", err)
	}
...
func (w *workspace) prepare(options engine.WorkspaceOptions) error {
	...
	if len(options.Artifacts.Named) > 0 {
		mounted, err := w.mountNamedArtifacts(options.Artifacts.Named)
		if err != nil {
			return err
		}
...
func (w *workspace) mountNamedArtifacts(layers map[string]string) (string, error) {
...
func (w *workspace) mountDirectory(name, source, target string) (string, error) {
	...
	if err = w.access.MountArtifact(absolute, target); err != nil {
...
func (w *workspace) WriteFile(name string, content []byte, mode os.FileMode) (string, error) {
	...
	if err := w.access.Own(path); err != nil {
		return "", err
	}
	return path, nil
}
...
func (w *workspace) mkdirTask(path string, mode os.FileMode) error {
	...
	current := w.root
	if err = w.access.Own(current); err != nil {
		return err
	}
	if relative == "." {
		return nil

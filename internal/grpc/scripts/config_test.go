...
func TestSandboxConfigResolve(t *testing.T) {
	tests := []struct {
		...
		{name: "root 下 auto 默认降权", euid: 0, enabled: true, uid: 65534, gid: 65534},
		{name: "非 root 下 auto 保持兼容", euid: 1000},
		{name: "off 显式关闭", config: SandboxConfig{Mode: SandboxModeOff}, euid: 0},
		{name: "required 接受自定义身份", config: SandboxConfig{Mode: SandboxModeRequired, UID: 1001, GID: 1002}, euid: 0, enabled: true, uid: 1001, gid: 1002},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			sandbox, err := tt.config.resolve(tt.euid)
			if tt.wantError != "" {
				require.ErrorContains(t, err, tt.wantError)
				return
			}
			require.NoError(t, err)
			require.Equal(t, tt.enabled, sandbox.Enabled)
			require.Equal(t, tt.uid, sandbox.UID)
			require.Equal(t, tt.gid, sandbox.GID)
		})
	}
}
